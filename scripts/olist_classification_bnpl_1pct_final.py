...
def prepare_features(df, feature_set='C'):
    ...
    num_cols_temp = X_selected.select_dtypes(include=[np.number]).columns.tolist()
    ...
    cat_cols_temp = X_selected.select_dtypes(include=['object']).columns.tolist()
    for col in cat_cols_temp:
        if col in X_selected.columns:
            mode_val = X_selected[col].mode()[0] if not X_selected[col].mode().empty else 'Unknown'
            X_selected[col] = X_selected[col].fillna(mode_val)
    
    # Identify numerical and categorical columns (after filling, but types unchanged)
...
def main():
    ...
    if XGBOOST_AVAILABLE:
        models['XGBoost'] = xgb.XGBClassifier(random_state=42, n_estimators=50, eval_metric='logloss', n_jobs=-1)
