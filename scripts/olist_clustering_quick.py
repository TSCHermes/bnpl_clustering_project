...
# 5. Take a sample for quick testing (10% of customers)
print("Taking a 10% sample for quick testing...")
customer_features_sample = customer_features.sample(frac=0.1, random_state=42)
...
# 8. Apply PCA for dimensionality reduction (to 10 components for speed)
...
output_path = data_path / 'olist_customer_clusters_quick_sample.csv'
customer_features_sample.to_csv(output_path, index=False)
print(f"\nSaved customer features with clusters (sample) to {output_path}")

# 13. Generate report
report_path = data_path / 'OLIST_CLUSTERING_REPORT_QUICK.md'
with open(report_path, 'w') as f:
    f.write("# Olist E-commerce Customer Clustering Report (Quick Sample)\\n\\n")
    f.write(f"Generated on: {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}\\n\\n")
    f.write(f"**Note**: This analysis is based on a 10% random sample of customers for quick testing.\\n\\n")
...
try:
    ...
    plt.title(f'Olist Customer Segmentation (k={best_k}): Recency vs Monetary (Quick Sample)')
    plt.grid(True, alpha=0.3)
    plot_path = data_path / 'olist_cluster_scatter_recency_monetary_quick.png'
    plt.tight_layout()
