...
customer_features = orders_items_products_sellers_payments_reviews.groupby('customer_unique_id').agg(
    # RFM features
    recency=('order_purchase_timestamp', lambda x: (data_path / 'dummy').stat().st_mtime - x.max().timestamp()),  # Will fix below
...
customer_features['recency'] = (latest_order_date - customer_features['recency']).dt.days
...
# 8. Clustering with MiniBatchKMeans (more efficient)
...
# 9. Select best k based on silhouette score
...
# 10. Add cluster labels to customer features
customer_features['cluster'] = best['labels']

# 11. Save results
output_path = data_path / 'olist_customer_clusters_pca.csv'
customer_features.to_csv(output_path, index=False)
print(f"\nSaved customer features with clusters to {output_path}")

# 12. Generate report
report_path = data_path / 'OLIST_CLUSTERING_REPORT_PCA.md'
with open(report_path, 'w') as f:
    f.write("# Olist E-commerce Customer Clustering Report (with PCA)\\n\\n")
...
# 13. Create a simple scatter plot of two key features (using original features for interpretability)
try:
    ...
    plt.title(f'Olist Customer Segmentation (k={best_k}): Recency vs Monetary (PCA-reduced features)')
    plt.grid(True, alpha=0.3)
    plot_path = data_path / 'olist_cluster_scatter_recency_monetary_pca.png'
    plt.tight_layout()
