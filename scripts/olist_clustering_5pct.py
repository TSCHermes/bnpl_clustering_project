...
print("Step 1: Loading customers and taking a sample...")
...
# Sample 5% of customers for balance
sample_frac = 0.05
...
print("\nStep 12: Applying PCA for dimensionality reduction (to 90% variance for speed)...")
...
print("\nStep 13: Clustering with MiniBatchKMeans (evaluating k=2 to 4)...")
results = []
for k in range(2, 5):  # k=2,3,4
    kmeans = MiniBatchKMeans(n_clusters=k, random_state=42, batch_size=500, max_iter=30)
...
# Save results
output_path = data_path / 'data/processed' / f'olist_customer_clusters_sampled_{int(sample_frac*100)}pct.csv'
...
# Generate report
report_path = data_path / 'docs' / f'OLIST_CLUSTERING_REPORT_SAMPLED_{int(sample_frac*100)}PCT.md'
with open(report_path, 'w') as f:
    f.write(f"# Olist E-commerce Customer Clustering Report ({sample_frac*100}% Sample)\n\n")
    f.write(f"Generated on: {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
    f.write(f"**Note**: This analysis is based on a {sample_frac*100}% random sample of customers for analysis.\n\n")
...
try:
    ...
    plot_path = data_path / 'results/visuals' / f'olist_cluster_scatter_recency_monetary_sampled_{int(sample_frac*100)}pct.png'
    plt.tight_layout()
