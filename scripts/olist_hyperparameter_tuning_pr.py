...
def main():
    ...
    for model_name, model, param_dist in models_params:
        ...
        # Randomized search with n_jobs=1 to avoid multiprocessing issues
        ...
        default_auc_roc = roc_auc_score(y_test, y_pred_proba_default)
        default_auc_pr = average_precision_score(y_test, y_pred_proba_default)
        ...
        result = {
            ...
            'default_auc_roc': default_auc_roc,
            'default_auc_pr': default_auc_pr,
        ...
        print(f"Default AUC-ROC: {default_auc_roc:.4f}")
        print(f"Default AUC-PR: {default_auc_pr:.4f}")
        print(f"Improvement AUC-PR: {test_auc_pr - default_auc_pr:.4f}")
    ...
    for res in results:
        print(f"{res['model']}:")
        print(f"  Default AUC-PR: {res['default_auc_pr']:.4f}")
        print(f"  Best CV AUC-PR: {res['best_cv_auc_pr']:.4f}")
        print(f"  Test AUC-PR: {res['test_auc_pr']:.4f}")
        print(f"  Improvement: {res['test_auc_pr'] - res['default_auc_pr']:.4f}")
        print()
